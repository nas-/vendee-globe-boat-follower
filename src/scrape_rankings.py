...
def get_rankings():
    soup = BeautifulSoup(
        requests.get("https://www.vendeeglobe.org/en/ranking").text, "html.parser"
    )
    ...
    year, month, day = map(
        int, re.findall(r"(202\d)(\d{2})(\d{2})", Current_ranking)[0]
    )
    try:
    ...
    df["ELAPSED"] = (
        pd.to_datetime(f"{year}-{month}-{day} " + df.ELAPSED)
        .dt.tz_localize("Europe/Paris")
        .dt.tz_convert(get_localzone().zone)
        .astype("int64")
        // 10**9
    )
    ...
    for element in rankings:
        ...
        if series:
            if position["ELAPSED"] > series[-1]["ELAPSED"]:
                ...
                logger.debug(
                    f"not appending  {element} {position['ELAPSED']} {series[-1]['ELAPSED']}"
                )
        else:
