...
class Scrape(cfscrape.Session):
    ...
    def request_marinetraffic_coordinate(self, x, y, zoom, do_req=False, **kwargs):
        ...
        if zoom == 10:
            ...
            Y = round(
                Y_coeff[0] + Y_coeff[1] * y + Y_coeff[2] * y**2 + Y_coeff[3] * y**3
            )
        ...
        Ys = [Y - 1, Y, Y + 1]
        urls = [
            f"https://www.marinetraffic.com/getData/get_data_json_4/z:{zoom}/X:{X_str}/Y:{Y_str}/station:0"
            for X_str in Xs
            for Y_str in Ys
        ]
        headers = {
        ...
        for url in urls:
            ...
            if do_req:
                ...
                try:
                    ...
                    logger.warning(
                        f"MarineTraffic returned {session_attempt.status_code} - {err}"
                    )
    ...
    def request_marinetrafic_url(self, url, **kwargs):
        ...
        zoom = int(zoom)
        return self.request_marinetraffic_coordinate(
            longitudex, latitudey, zoom, **kwargs
        )
